...
pub struct CallLogger {
    ...
    call_back: Option<&'static dyn Fn(&str)>,
}

static MY_LOGGER: spin::Mutex<RefCell<Option<CallLogger>>> = spin::Mutex::<RefCell::new(None)>;
...
impl<F> CallLogger<F> {
    ...
    #[inline]
    #[must_use = "You must call init() before logging"]
    pub fn with_call(mut self, call_back: &'static dyn Fn(&str)) -> CallLogger {
